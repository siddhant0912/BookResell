import React, {useState} from 'react'
import { connect } from 'react-redux'
import PropTypes from 'prop-types'
import {SearchBooks} from '../Actions/Books'
import BookItem from '../Components/Books/BookItem'
import { Fragment } from 'react'

const SearchScreen = ({SearchBooks,book:{loading,searchResults}}) =>{
    const [query, setQuery] = useState('')
    const onChange = e => {
        setQuery(e.target.value)
        SearchBooks(e.target.value)
    }
    
    return(
        <Fragment>
        <div className="wrapper">
            <img className="search-icon" alt="" src="data:image/svg+xml;utf8;base64," />
            <input className="search" placeholder="Search......" type="search" value={query} onChange={e => onChange(e)} />
        </div>
        <ol className="books-grid">
                {searchResults.map(book =>(
                    <BookItem key={book._id} book={book}/>
                ))}
           </ol>
        </Fragment>



    )
}
const mapStateToProps = state =>({
    book:state.Books
})

SearchScreen.propTypes ={
    SearchBooks:PropTypes.func.isRequired,
}
export default connect(mapStateToProps, {SearchBooks})(SearchScreen)